...
def parse_document(text):
    messages = [
        {"role": "system", "content": f"Given the following text from a resume, identify and structure the company name, job title, hours worked, time period, and related bullets for each job experience. The text is:\n\n{text}\n\n PERSERVE THE START/END indexs of items parsed, Return the structured data in JSON format with the following keys:START/END INDEX(can be multiple in one key) company_name, job_title, hours_worked, time_period, details (list of bullets). "}]
        ...
        if 'paragraph' in element:
            ...
            for paragraph_element in paragraph.get('elements'):
                ...
                if text_run:
                    ...
                    # Embedding indices within the text
                ...
                if 'bullet' in paragraph:
                    ...
                    formatting_details[-1].update({
                        'list_id': list_id,
                        'nesting_level': nesting_level
                    })
    print(text_with_indices)
    parsed_document = json.loads(parse_document(text_with_indices))
    print(parsed_document)
    changes = []

    for items in parsed_document.values():
        if isinstance(items, list):
            for item in items:
                if isinstance(item, dict):
                    for key in item:
                        if key == 'details' and isinstance(item[key], list):
                            for i, detail in enumerate(item[key]):
                                detail_text = ""
                                detail_formatting = []
                                for fmt in formatting_details:
                                    if detail in fmt['text']:
                                        detail_text += fmt['text']
                                        detail_formatting.append(fmt)
                                if detail_text == detail:
                                    changes.append((item[key], i, detail_formatting))
                        else:
                            item_text = ""
                            item_formatting = []
                            for fmt in formatting_details:
                                if item[key] in fmt['text']:
                                    item_text += fmt['text']
                                    item_formatting.append(fmt)
                            if item_text == item[key]:
                                changes.append((item, f"{key}_formatting", item_formatting))
...
if __name__ == "__main__":
    ...
    # Example: Access a specific job and its details
    ...
    for job in experience_section:
        ...
        company_name_formatting = job.get('company_name_formatting', [])
        for fmt in company_name_formatting:
            print(f"  Text: {fmt['text']}")
            print(f"  Start Index: {fmt['start_index']}")
            print(f"  End Index: {fmt['end_index']}")
            print(f"  Text Style: {fmt['text_style']}")
            if 'fontSize' in fmt['text_style']:
                print(f"  Font Size: {fmt['text_style']['fontSize']['magnitude']} {fmt['text_style']['fontSize']['unit']}")
            print()
